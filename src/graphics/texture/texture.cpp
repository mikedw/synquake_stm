 ...
 *
 *********************************************************/
int LoadBitmap(char *filename, int *w, int *h)
{
    FILE * file;
    char temp;
    long i;

    BITMAPINFOHEADER2 infoheader;
    ...
    *h = infoheader.biHeight;
    ...
    //printf( "%s   %d(%x) %d(%x) %d(%x) \n", filename, infoheader.biWidth, infoheader.biWidth, infoheader.biHeight, infoheader.biHeight, 
	//			infoheader.biPlanes, infoheader.biPlanes  );
    ...
    // read the data.
    infoheader.data = (char *) malloc_wrapper(infoheader.biWidth * infoheader.biHeight * 3);
    ...
    free_wrapper(infoheader.data); // Free the memory we used to load the texture
